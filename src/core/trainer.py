...
class BaseTrainer(object):
    ...
    def __init__(self, model: nn.Module, parameters: dict, device: str):
        ...
        self.early_stop = EarlyStopping(
            patience=parameters['early_stopping_patience'], enable_wandb=parameters['track'])
        ...
        for epoch in range(num_epochs):
            ...
            if self.parameters['track']:
                wandb.log({
                    ...
                    f"Train/Metric": train_loss,
                    f"Test/Metric": test_loss,
                    "_step_": epoch
                })
            ...
            logging.info(
                f"Epoch {epoch + 1} / {num_epochs} - Train/Test {self.parameters['loss']['class_name']}: {train_loss:.4f} | {test_loss:.4f} - Train/Test Metric: {train_metric:.4f} | {test_metric:.4f}")
